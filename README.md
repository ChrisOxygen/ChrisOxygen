# Hi, I'm Chris 

Full-stack developer specializing in building production-ready web applications with **Next.js**, **React**, and **TypeScript**. I focus on taking projects from concept to launch, with particular interest in AI integration and SaaS products.

##  About Me

-  3+ years of experience in full-stack development
-  Background in UI/UX design, bringing strong design sensibility to my code
-  Expertise in building MVPs and taking applications to production
-  Based in Nigeria, working with clients globally
-  Delivered projects with real user traction (500+ active users)

##  Tech Stack

![Next.js](https://img.shields.io/badge/Next.js-000000?style=for-the-badge&logo=nextdotjs&logoColor=white)
![React](https://img.shields.io/badge/React-61DAFB?style=for-the-badge&logo=react&logoColor=black)
![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=for-the-badge&logo=typescript&logoColor=white)
![Tailwind CSS](https://img.shields.io/badge/Tailwind_CSS-38B2AC?style=for-the-badge&logo=tailwind-css&logoColor=white)
![Supabase](https://img.shields.io/badge/Supabase-3ECF8E?style=for-the-badge&logo=supabase&logoColor=white)
![Node.js](https://img.shields.io/badge/Node.js-339933?style=for-the-badge&logo=nodedotjs&logoColor=white)
![Prisma](https://img.shields.io/badge/Prisma-2D3748?style=for-the-badge&logo=prisma&logoColor=white)

**Other tools**: OpenAI API, Stripe, Vercel, Git, Figma

---

##  Featured Projects

### [ProPreso](https://propreso.com)
Chrome extension for AI-powered presentation enhancement  
**Tech**: Next.js, TypeScript, OpenAI API  
**Impact**: 50+ downloads

### [Invox](https://invox.cc)
Complete invoicing platform for freelancers and small businesses  
**Tech**: Next.js, Supabase, Stripe integration  
**Features**: Invoice generation, payment tracking, client management

### SnapIgnite
Social media management and content creation tool  
**Tech**: Next.js, React, TypeScript, AI integration

---

##  What I'm Working On

-  Building a sophisticated visa application platform with admin review systems
-  Exploring AI integration patterns for production applications
-  Developing marketing automation tools

---

##  GitHub Activity

[![GitHub Streak](https://streak-stats.demolab.com?user=ChrisOxygen&theme=dark&hide_border=true&background=0D1117)](https://git.io/streak-stats)

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=ChrisOxygen&layout=compact&theme=dark&hide_border=true&bg_color=0D1117)

---

##  Let's Connect

-  [LinkedIn](https://www.linkedin.com/in/christopher-okafor-17084416b)
-  Portfolio: [https://usechris.dev](https://usechris.dev)
-  Email: chris@usechris.dev

---

**Open to**: Full-time Software Engineer and Frontend Developer roles | Freelance projects | Collaboration opportunities
